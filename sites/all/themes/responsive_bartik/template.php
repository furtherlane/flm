...
function responsive_bartik_preprocess_html(&$variables)
{
  ...
  //Add flm.css here, so that we can use 'weight'
  if (file_exists(drupal_get_path('theme', 'responsive_bartik') . '/css/flm.css')) {
  	drupal_add_css(drupal_get_path('theme', 'responsive_bartik') . '/css/flm.css',
  			array('type' => 'file', 'weight' => 1));
  }
